...
public class Constraints_dAB_dAC_dBF extends OperatorABCDEF {

	@Override
	public void init() {
		constraints = "dAB_dAC_dBF";
	}
	...
	public double proposal(double width, double ta, double tb, double tc, double td, double te, double tf, double ra, double rb, double rc, double rd, double re) {
		...
		rcp = -(tf*(r_epsilon + re) + rc*tc - rc*te + 2*rd*td - 2*rd*te + re*te - re*tf - (r_delta + rd)*(r_zeta + td) + (r_delta + rd)*(r_eta + te) - (r_epsilon + re)*(r_eta + te))/(r_zeta - tc + td);
		rdp = r_delta + rd;
		rep = r_epsilon + re;
		tdp = r_zeta + td;
		tep = r_eta + te;
