...
public class Constraints_dAB_dAC_dBC_dBF extends OperatorABCDEF {

	@Override
	public void init() {
		constraints = "dAB_dAC_dBC_dBF";
	}
	...
	public double proposal(double width, double ta, double tb, double tc, double td, double te, double tf, double ra, double rb, double rc, double rd, double re) {
		...
		rdp = (r_eta*r_epsilon + r_eta*re + r_epsilon*te - r_epsilon*tf - rd*td + rd*te)/(r_eta - r_zeta - td + te);
		rep = r_epsilon + re;
		tdp = r_zeta + td;
		tep = r_eta + te;
