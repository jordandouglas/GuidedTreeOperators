...
public class Constraints_dAB_dAE_dBC extends OperatorABCDE {

	@Override
	public void init() {
		constraints = "dAB_dAE_dBC";
	}
	...
	public double proposal(double width, double ta, double tb, double tc, double td, double te, double ra, double rb, double rc, double rd) {
		...
		rcp = (te*(r_delta + rd) - rc*tc + rc*te - 2*rd*td + 2*rd*te - (r_delta + rd)*(r_epsilon + td))/(r_epsilon - tc + td);
		rdp = r_delta + rd;
		tdp = r_epsilon + td;
